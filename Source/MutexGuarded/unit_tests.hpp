...
TEST_CASE("Simple sanity checks")
{
   ...
   {
      REQUIRE(std::is_same_v<mutex_guarded<std::string, std::mutex>::value_type, std::string>);
      REQUIRE(std::is_same_v<mutex_guarded<std::string, std::mutex>::reference, std::string&>);
   }
}
